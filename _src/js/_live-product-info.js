const LiveProductInfo = (function () {
  return {
    ...
    selectors: {
      ...
      productPriceQuantityFieldForLists:
        ".item_swift_productlistcompactview .js-quantity, .item_swift_productlistlistview .js-quantity",
      productPricePrice: ".js-text-price-price",
      quantityField: ".swift_quantity-field",
      stock: ".js-text-stock",
      expectedDelivery: ".js-text-expected-delivery",
      stockMessages:
        ".js-stock-state div, .js-stock-state small, .js-stock-state p",
    ...
    init: function () {
      ...
      const relatedProducts = document.querySelector(
        self.selectors.relatedProducts
      );
      if (relatedProducts) {
      ...
      document.addEventListener("afterSwap.swift.productlist", function () {
        const feedUrlElements = document.querySelectorAll(
          LiveProductInfo.selectors.productInfoFeed
        );
        ...
        if (feedUrlElements.length) {
          const content = document.querySelector(
            LiveProductInfo.selectors.content
          );
    ...
    UpdateProductInfo: function (selector) {
      const self = this;
      const feedUrlElements = selector
        ? selector.querySelectorAll(self.selectors.productInfoFeed)
        : document.querySelectorAll(self.selectors.productInfoFeed);
      const liveInfoContainers = document.querySelectorAll(
        self.selectors.liveInfo
      );
      ...
      feedUrlElements.forEach(function (el) {
        ...
        const query = new URLSearchParams(
          self.SanitizeParameters(feedUrlArray[1])
        );
        let fetchProducts = {
          headers: {
        ...
        if (bearerToken !== "") {
          ...
          console.error(
            "The Bearer Token cookie is missing. You may not have the necessary dll installed. Disable 'Lazy Loading' of Live product info to get updated data."
          );
        }

        const isPostMethod = query.has("RepositoryName");
        if (isPostMethod) {
          const productIds = getValueAndUpdateQuery(
            query,
            "ProductIds",
            query.getAll("ProductIds").join(","),
            el.getAttribute(self.config.productIdAttr)
          );
          const pageSize = getValueAndUpdateQuery(
            query,
            "PageSize",
            parseInt(query.get("PageSize")),
            100
          );
          const currencyCode = getValueAndUpdateQuery(
            query,
            "CurrencyCode",
            query.get("CurrencyCode"),
            ""
          );
          const countryCode = getValueAndUpdateQuery(
            query,
            "CountryCode",
            query.get("CountryCode"),
            ""
          );
          const languageId = getValueAndUpdateQuery(
            query,
            "LanguageId",
          ...
          fetchProducts.body = JSON.stringify({
            PageSize: pageSize,
            Parameters: { MainProductID: productIds },
    ...
    UpdateValues: function (data, liveInfoContainers) {
      ...
      function removeLoaders(container) {
        container
          .querySelectorAll(self.selectors.loader)
          .forEach(function (el) {
      ...
      function setProductPrice(container, product) {
        if (
          product.Price != null &&
          container.querySelectorAll(self.selectors.price)
        ) {
        ...
        function setPrice(container, product) {
            ...
            if (product.VariantInfo != null && isPdpWithVariantId()) {
              if (
                ...
                price =
                  ...
                  " - " +
                  product.VariantInfo.PriceMax.PriceFormatted;
              }
            }
          ...
          self.UpdateDataAttribute(
            container.querySelectorAll(self.selectors.priceFormatted),
            self.config.priceFormattedAttr,
            product.Price.PriceFormatted
          );
          self.UpdateDataAttribute(
            container.querySelectorAll(self.selectors.priceProp),
            self.config.contentAttr,
            product.Price.Price
          );
        }

        function setPriceWithVat(container, product) {
          const pricesWithVatContainers = container.querySelectorAll(
            self.selectors.priceWithVat
          );
          const isMasterProduct = product.VariantInfo.Price != null;
          const productPriceWithVat = isMasterProduct
            ? product.VariantInfo.Price.PriceWithVat
            : product.Price.PriceWithVat;
          const productPriceWithVatFormatted = isMasterProduct
            ? product.VariantInfo.Price.PriceWithVatFormatted
            : product.Price.PriceWithVatFormatted;

          if (
            productPriceWithVat != null &&
            pricesWithVatContainers.length > 0
          ) {
            ...
            if (product.VariantInfo != null) {
              if (
                ...
                priceWithVat =
                  ...
                  " - " +
                  product.VariantInfo.PriceMax.PriceWithVatFormatted;
              }
            }

            priceWithVat = getDataSuffix(
              container,
              self.selectors.priceWithVat,
              priceWithVat
            );
        ...
        function setPriceBeforeDiscount(container, product) {
          const priceBeforeDiscountContainers = container.querySelectorAll(
            self.selectors.priceBeforeDiscount
          );
          if (
            product.PriceBeforeDiscount != null &&
            priceBeforeDiscountContainers.length > 0
          ) {
            self.UpdateValue(
              priceBeforeDiscountContainers,
        ...
        function isPdpWithVariantId() {
          ...
          if (linkElement) {
            ...
            const variantId = self.GetValue(
              parseInt(query.get("VariantID")),
              ""
            );
      ...
      function setPriceMatrix(container, product) {
        if (
          product.Prices != null &&
          product.Prices &&
          container.querySelectorAll(self.selectors.productPriceQuantity) &&
          container.querySelectorAll(self.selectors.productPricePrice)
        ) {
          let productPricesContainers = container.querySelectorAll(
            self.selectors.productPricesContainer
          );

          productPricesContainers.forEach(function (productPricesTopContainer) {
            let productPricesContainer =
              productPricesTopContainer.querySelector(
                self.selectors.productPrices
              );
            let template = productPricesContainer.querySelector(
              self.selectors.productPriceTemplate
            );
            ...
            product.Prices.forEach(function (quantityPrice) {
              ...
              self.UpdateValue(
                priceContainer.querySelectorAll(
                  self.selectors.productPriceQuantity
                ),
                quantityPrice.Quantity
              );
              self.UpdateValue(
                priceContainer.querySelectorAll(
                  self.selectors.productPricePrice
                ),
                quantityPrice.Price.PriceFormatted
              );
              self.ShowConditionalElement(
                priceContainer.querySelectorAll(self.selectors.showIfAttr)
              );
      ...
      function setStockLevel(container, product) {
        const stockContainers = container.querySelectorAll(
          self.selectors.stock
        );
        if (product.StockLevel != null && stockContainers.length > 0) {
          self.UpdateValue(stockContainers, product.StockLevel);
          self.ShowConditionalElement(
            container.querySelectorAll(self.selectors.stockMessages)
          );
        }
      ...
      function setExpectedDelivery(container, product) {
        const expectedDeliveryContainers = container.querySelectorAll(
          self.selectors.expectedDelivery
        );
        if (
          product.ExpectedDelivery != null &&
          expectedDeliveryContainers.length > 0
        ) {
          self.UpdateValue(
            expectedDeliveryContainers,
            product.ExpectedDelivery
          );
          self.ShowConditionalElement(
            container.querySelectorAll(self.selectors.expectedDelivery)
          );
        }
      }

      function updateVariantSelector(container) {
        const variantSelector = container.querySelector(
          self.selectors.variantSelector
        );
      ...
      function enableQuantityFieldAndAddToCart(container, product) {
        if (product.StockLevel && product.StockLevel > 0) {
          container
            .querySelectorAll(self.selectors.productPriceQuantityFieldForLists)
            .forEach(function (el) {
              el.disabled = false;
            });
          container
            .querySelectorAll(self.selectors.quantityField)
            .forEach(function (el) {
              el.disabled = false;
            });
          container
            .querySelectorAll(self.selectors.addToCart)
            .forEach(function (el) {
              el.disabled = false;
            });
        }
      ...
      function showLoadMore() {
        document
          .querySelectorAll(self.selectors.loadMoreButton)
          .forEach(function (el) {
    ...
    GetProductData: function (container, data) {
      ...
      const variantId = container.getAttribute(
        self.config.productVariantIdAttr
      );
