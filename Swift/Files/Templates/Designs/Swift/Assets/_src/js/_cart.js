	...
	return {
		Update: async function (e) {
			...
			if (globalDispatcher != false && localDispatcher != false) {
				...
				if (formData.get("minicartid") == null) {
					document.querySelectorAll(".js-cart-qty").forEach(function (el) {
						el.classList.add("mini-cart-quantity-added");
					});
				} else {
					document.querySelector("#Cart_" + formData.get("minicartid")).classList.add("mini-cart-quantity-added");
				}
			...
			if (globalDispatcher != false && localDispatcher != false) {
				...
				var removeFocusCssClassTimer = setTimeout(function () {
					if (formData.get("minicartid") == null) {
						...
						document.querySelector("#Cart_" + formData.get("minicartid")).classList.remove("mini-cart-quantity-added");
					}
				}, 200);
				...
				if (formData.get("minicartid") == null) {
					...
					document.querySelector("#Cart_" + formData.get("minicartid")).innerHTML = "(" + totalQuantity.trim() + ")";
				}
			}
		},
